...
class Solution
  ...
  def clear_collection
    #place solution here
    @@db[RACE_COLLECTION].find().delete_many()
  end

  def load_collection(file_path)
    #place solution here
    race_hash =  Solution.load_hash(file_path)
    @@db[RACE_COLLECTION].insert_many(race_hash)
  end
  ...
  def find_by_name(fname, lname)
    #place solution here
    @@db[RACE_COLLECTION].find(:first_name => fname, :last_name => lname).projection(:_id => false, :first_name => true, :last_name => true, :number => true)
  end
  ...
  def find_group_results(group, offset, limit)
    #place solution here
    @@db[RACE_COLLECTION].find(:group => group).projection(:_id => false, :group => true, :number => true, :first_name => true, :last_name => true, :gender => true, :secs => true).sort({:secs => 1}).skip(offset).limit(limit)
  end
  ...
  def find_between(min, max)
    #place solution here
    @@db[RACE_COLLECTION].find(:secs => {:$lt => max, :$gt => min})
  end

  def find_by_letter(letter, offset, limit)
    #place solution here
    @@db[RACE_COLLECTION].find(:last_name => {:$regex => "^#{letter}"}).sort({:last_name => 1}).skip(offset).limit(limit)
  end
  ...
  def update_racer(racer)
    #place solution here
    @@db[RACE_COLLECTION].find(:_id => racer[:_id]).replace_one(racer)
  end

  def add_time(number, secs)
    #place solution here
    @@db[RACE_COLLECTION].find(:number => number).update_one(:$inc => {secs: secs })
  end
